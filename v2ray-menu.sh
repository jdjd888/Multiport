...
EOF
    ...
    cat /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","alterId": 0,"add": "'${domain}'","email": "'${email}'"}]' > /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds_tmp.json
	mv -f /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds_tmp.json /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds.json
	cat <<EOF >>"/etc/rare/config-user/${user}"
vless://$uuid@$sub.$domain:$tls?flow=xtls-rprx-splice&encryption=none&security=tls&sni=$sni&type=tcp&headerType=none&host=$sni#$user
vless://$uuid@$sub.$domain:$tls?flow=xtls-rprx-direct&encryption=none&security=tls&sni=$sni&type=tcp&headerType=none&host=$sni#$user
vless://$uuid@$sub.$domain:$tls?encryption=none&security=tls&sni=$sni&type=ws&host=$sni&path=/v2rayws#$user
trojan://$uuid@$sub.$domain:$tls?sni=$sni#$user
${vmesslink1}
EOF
...
# =======================================
proxies:
  - {name: trojan_${user}, server: $sub.$domain, port: $tls, type: trojan, password: $uuid, sni: $sni, skip-cert-verify: true, udp: true}
  - {name: vmess_${user}, server: $sub.$domain, port: $tls, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: false, skip-cert-verify: true, network: ws, ws-path: /v2rayvws, ws-headers: {Host: $sni}, udp: true}
port: 3333
socks-port: 23456
redir-port: 23457
allow-lan: true
...
EOF
    ...
    echo -e "\E[0;100;33m   • V2RAY USER INFORMATION •      \E[0m" | tee -a /etc/log-create-user.log
    echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m" | tee -a /etc/log-create-user.log
    ...
    echo -e "" | tee -a /etc/log-create-user.log
	echo -e " Link VLESS SPLICE: vless://$uuid@$sub.$domain:$tls?flow=xtls-rprx-splice&encryption=none&security=tls&sni=$sni&type=tcp&headerType=none&host=$sni#$user" | tee -a /etc/log-create-user.log
    echo -e "" | tee -a /etc/log-create-user.log
    echo -e " Link VLESS DIRECT: vless://$uuid@$sub.$domain:$tls?flow=xtls-rprx-direct&encryption=none&security=tls&sni=$sni&type=tcp&headerType=none&host=$sni#$user" | tee -a /etc/log-create-user.log
    echo -e "" | tee -a /etc/log-create-user.log
	echo -e " Link VLESS WS: vless://$uuid@$sub.$domain:$tls?encryption=none&security=tls&sni=$sni&type=ws&host=$sni&path=/v2rayws#$user" | tee -a /etc/log-create-user.log
    echo -e "" | tee -a /etc/log-create-user.log
	echo -e " Link TROJAN: trojan://$uuid@$sub.$domain:$tls?sni=$sni#$user" | tee -a /etc/log-create-user.log
    echo -e "" | tee -a /etc/log-create-user.log
    echo -e " Link VMESS NONE TLS: ${vmesslink1}" | tee -a /etc/log-create-user.log
	echo -e "" | tee -a /etc/log-create-user.log
    echo -e " Link url OPENWRT/V2rayN PC: https://${domain}/s/${uuid}" | tee -a /etc/log-create-user.log
	echo -e "" | tee -a /etc/log-create-user.log
	echo -e " Link url Android Clash: https://${domain}/s/${user}" | tee -a /etc/log-create-user.log
	echo -e "" | tee -a /etc/log-create-user.log
	echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m" | tee -a /etc/log-create-user.log
    echo ""
    read -n 1 -s -r -p "Press any key to back on menu"
...
function delete-user() {
	...
	mv -f /etc/rare/v2ray/conf/04_trojan_TCP_inbounds_tmp.json /etc/rare/v2ray/conf/04_trojan_TCP_inbounds.json		
    cat /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds.json | jq 'del(.inbounds[0].settings.clients[] | select(.id == "'${uuid}'"))' > /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds_tmp.json
	mv -f /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds_tmp.json /etc/rare/v2ray/conf/05_VMess_NONE_WS_inbounds.json
...
function change-port() {
    ...
    sed -i "s/   - V2RAY VMESS NONE TLS         : $tls/   - V2RAY VMESS NONE TLS         : $tls1/g" /root/log-install.txt
...
0) clear ; menu ;;	
x) exit ;;
*) echo -e "" ; echo "Masukkan number yang betul" ; sleep 1 ; v2ray-menu ;;
esac
